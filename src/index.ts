...
extendEnvironment((hre) => {
  ...
  hre.getNetwork = async function getNetwork(network: string) {
    if (!this.config.networks[network]) {
      throw new Error(`changeNetwork: Couldn't find network '${network}'`);
    }

    return {
      name: network,
      config: this.config.networks[network],
      provider: await this.getProvider(network),
    }
  }
});
